...
public class PVHost {
    ...
    private void handleIncomingMessages() {
        ...
        try {
            ...
            byte[] buffer = new byte[HyperdriveConstants.SOCKET_BUFFER_SIZE];
            ...
            while(currentData.indexOf(HyperdriveConstants.START_SEQUENCE) > -1 && currentData.indexOf(HyperdriveConstants.END_SEQUENCE) > -1) {
                int
                    startSequenceIndex = currentData.indexOf(HyperdriveConstants.START_SEQUENCE),
                    endSequenceIndex = currentData.indexOf(HyperdriveConstants.END_SEQUENCE);
                ...
                String[] completedMessages = relavantData.split(HyperdriveConstants.END_SEQUENCE);
                for(int i=0; i<completedMessages.length; i++) {
                    String completedMessage = completedMessages[i];

                    int splitSequenceIndex = completedMessage.indexOf(HyperdriveConstants.SPLIT_SEQUENCE);
                    if(splitSequenceIndex > -1) { //now it is defintely a full message
                        String subject = completedMessage.substring(0, splitSequenceIndex);
                        String message = completedMessage.substring(splitSequenceIndex + HyperdriveConstants.SPLIT_SEQUENCE.length());
                        String extraInfo = "";

                        if(subject.startsWith(HyperdriveConstants.START_SEQUENCE)) {
                            subject = subject.substring(HyperdriveConstants.START_SEQUENCE.length()); //substring off "("
                        }

                        if(subject.contains(HyperdriveConstants.SUBJECT_SEQUENCE)) {
                            extraInfo = subject.substring(subject.indexOf(HyperdriveConstants.SUBJECT_SEQUENCE) + HyperdriveConstants.SUBJECT_SEQUENCE.length());
                        }
    ...
    private void handleMessage(String subject, String message, String extraInfo) {
        ...
        switch(messageType) {
            case DIRECTORY_REQUEST: { //return a message with all contents of the directory separated by newlines
                    String[] paths = HyperdriveUtil.getFilesInDirectory(message, true);
